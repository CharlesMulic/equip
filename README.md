...
It now also preserves a machine-readable `changesets-release-result.json` artifact, a final `changesets-release-assertion.json` gate verdict, a rewritten `changesets-release-summary.md` artifact that includes that final assertion state plus the uploaded artifact names to open next, and a canonical `changesets-release-report.json` rollup that ties the result, assertion, artifact paths, and uploaded artifact names together. The final Changesets assertion artifact now also carries its own evidence paths/names, the upstream release-verification input-presence state, plus the actual/effective status split, so the last publish gate file is self-contained too instead of sending operators back to the larger report immediately. When the publish lane is blocked by an earlier verification failure, that evidence now records the Changesets step as `skipped` with the upstream gate reason instead of flattening it into a generic failure. The canonical report preserves that split explicitly: `status` reflects the actual lane outcome, while `effectiveStatus` reflects the final gate interpretation after assertion. The Changesets summary/report/assertion path now also preserves whether the result artifact itself was present, so even a broken or missing result writer still leaves behind a truthful `missing` evidence bundle instead of failing before the final publish artifacts exist. That means release-PR/publish failures leave behind both structured and quick-scan evidence instead of living only in workflow logs.
