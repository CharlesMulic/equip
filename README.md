...
The release workflow now starts with a dedicated bootstrap lane that runs `npm ci` through `scripts/ci/run-release-bootstrap.mjs`, preserving a machine-readable `release-bootstrap-result.json`, a human-readable `release-bootstrap-summary.md`, and a raw install log even when dependency install fails. That means release jobs no longer strand setup failures in raw Actions logs only before any later reporting can run. A dedicated preflight lane then runs the real `build` and `npm test` phases through `scripts/ci/run-release-preflight.mjs`, preserving a machine-readable `release-preflight-result.json`, a human-readable `release-preflight-summary.md`, and raw build/test logs even when one of those early phases fails. That means release jobs no longer strand the most important early build/test failures in raw Actions logs only. The pack verification lane then emits a machine-readable JSON report in CI, preserves a raw `pack-verification.log`, and uploads the actual packed `.tgz`, so release/publish failures point at the exact tarball contract that broke and leave behind both the inspected artifact and the raw `npm pack` output. That verification script now preserves those failure artifacts too, even when `npm pack` itself fails before it can produce normal metadata. A second tarball smoke lane now installs that exact `.tgz` into a clean temp project and proves the packaged `equip` / `unequip` CLIs plus the exported library entrypoint still work from the npm package boundary; it now also preserves both a machine-readable failure artifact and a raw `pack-install-smoke.log` when the install smoke dies early. CI and release then generate a single `release-verification-report.json` rollup that ties together the pack contract, tarball smoke, and Docker acceptance results, even when one of those upstream artifacts is missing because a prior verification lane failed early, and that rollup now rewrites its per-lane log/report/tarball pointers to the current verification workspace so the combined gate points straight at the downloaded evidence you can actually inspect from that job. It now also records the uploaded GitHub artifact names for those verification lanes, so downstream tooling can jump from the rollup to the right workflow artifact without guessing, and it now carries the bootstrap/preflight prerequisite state too so blocked downstream lanes show up as `skipped` with a real reason instead of being mislabeled `missing`. The final `release-verification-assertion.json` artifact now also carries that verification-lane context forward, including the machine-readable upstream input-presence state, bootstrap/preflight summaries, plus the package, tarball-smoke, and Docker evidence pointers, so the last verification gate file is self-contained instead of only listing verdicts. The final workflow rollup now includes both the bootstrap and preflight lanes, truthfully marks downstream verification and Changesets lanes as `skipped` when earlier release stages prevented them from running, preserves both the actual workflow status and the assertion-adjusted effective status in the canonical report, flattens the key nested log/report file paths into one top-level evidence-file view, now also flattens the nested verification and Changesets GitHub artifact names into one top-level evidence-artifact view, and now also carries the GitHub workflow context (repository/workflow/run/ref/sha/event plus derived run/commit URLs) through the canonical report, summary, and final assertion artifact so the outermost evidence bundle is self-describing outside Actions. It now carries those same evidence pointers plus the machine-readable input-presence state into the final `release-workflow-assertion.json` artifact so the last gate file is self-contained too. That outermost summary/assertion layer now also preserves a truthful `release-workflow-report missing` state instead of failing empty-handed if the final report artifact never appears, so the last release gate still leaves behind structured evidence in that edge case. It still fails explicitly if the combined report is not healthy, rewrites the final rollup so it points at the assertion and Markdown summary artifacts too, and uploads a final `release-verification-summary.md` artifact for the verification lane. The GitHub job summary itself now comes from the final top-level `release-workflow-summary.md`, so operators get one canonical human-readable release summary instead of a stack of partially overlapping sections.
...
It now also preserves a machine-readable `changesets-release-result.json` artifact, a final `changesets-release-assertion.json` gate verdict, a rewritten `changesets-release-summary.md` artifact that includes that final assertion state plus the uploaded artifact names to open next, and a canonical `changesets-release-report.json` rollup that ties the result, assertion, artifact paths, and uploaded artifact names together. The final Changesets assertion artifact now also carries its own evidence paths/names, the upstream release-verification input-presence state, plus the actual/effective status split, so the last publish gate file is self-contained too instead of sending operators back to the larger report immediately. Those Changesets lane artifacts now also carry the GitHub workflow context for that run, including repository/workflow/run/ref/sha/event plus derived run/commit URLs, so they stay actionable even when opened apart from the top-level workflow report. When the publish lane is blocked by an earlier verification failure, that evidence now records the Changesets step as `skipped` with the upstream gate reason instead of flattening it into a generic failure. The canonical report preserves that split explicitly: `status` reflects the actual lane outcome, while `effectiveStatus` reflects the final gate interpretation after assertion. The Changesets summary/report/assertion path now also preserves whether the result artifact itself was present, so even a broken or missing result writer still leaves behind a truthful `missing` evidence bundle instead of failing before the final publish artifacts exist. The release-verification report/summary/assertion artifacts now carry that same GitHub workflow context and derived URLs too, so the verification lane is equally self-describing outside the final top-level workflow bundle. That means release-PR/publish failures leave behind both structured and quick-scan evidence instead of living only in workflow logs.
